 ...
 * - Resetting of an end point (libusb_resetep())
 ...
 * Reset and end point.
 *
 * You should cancel all pending transfers before attempting to reset and end point.
 *
 * This is a blocking function.
 *
 * \param dev a device handle
 ...
 * \returns another LIBUSB_ERROR code on other failure
 */
API_EXPORTED int libusb_resetep(libusb_device_handle *dev,
	unsigned char endpoint)
{
	usbi_dbg("endpoint %x", endpoint);
	return usbi_backend->resetep(dev, endpoint);
}
	...
	const char *function, const char *format, va_list args)
{
...
#else // ~~99
	{
		...
		_vsnprintf(buf, 999, format, args);
#else
		vsnprintf(buf, 999, format, args);
#endif

		fprintf(stream, "libusb:%s th %d, tm %d %s\n", prefix, usbi_get_thread_id(), msec_time() - msec, buf);
	}
#endif
}
