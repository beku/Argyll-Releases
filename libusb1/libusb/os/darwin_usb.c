...
static int darwin_get_active_config_descriptor(struct libusb_device *dev, unsigned char *buffer, size_t len, int *host_endian) {
  ...
  config_value = priv->active_config;
...
static int darwin_set_configuration(struct libusb_device_handle *dev_handle, int config) {
  ...
  dpriv->active_config = (uint8_t)config;
...
static int darwin_resetep(struct libusb_device_handle *dev_handle, unsigned char endpoint) {
  struct darwin_device_handle_priv *priv = (struct darwin_device_handle_priv *)dev_handle->os_priv;
  ...
  kresult = (*(cInterface->interface))->ResetPipe(cInterface->interface, pipeRef);
  if (kresult)
    usbi_err (HANDLE_CTX (dev_handle), "ResetPipe: %s", darwin_error_str (kresult));
...
static int darwin_abort_transfers (struct usbi_transfer *itransfer) {
  ...
  /* (ClearPipeStallBothEnds kills some devices) */
  kresult = (*(cInterface->interface))->ClearPipeStall(cInterface->interface, pipeRef);
