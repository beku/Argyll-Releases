...
static int winusb_resetep(struct libusb_device_handle *dev_handle, unsigned char endpoint);
...
static int libusb0_set_config(struct libusb_device_handle *dev_handle, int config);
...
static int libusb0_resetep(struct libusb_device_handle *dev_handle, unsigned char endpoint);
...
static int hid_resetep(struct libusb_device_handle *dev_handle, unsigned char endpoint);
...
static int composite_resetep(struct libusb_device_handle *dev_handle, unsigned char endpoint);
	...
	if ( ++concurrent_usage == 0 ) {	// First init?
		...
		if ((GetVersionEx(&os_version) != 0) && (os_version.dwPlatformId == VER_PLATFORM_WIN32_NT)) {
			if ((os_version.dwMajorVersion == 5) && (os_version.dwMinorVersion == 0)) {
				...
				windows_version = WINDOWS_XP;	/* Or XP64, 2003 etc. */
...
enum libusb_hid_report_type {
	...
	for (i = 0; i<USB_MAXINTERFACES; i++)
	{
		...
		usbi_dbg("retrieved simple path '%s' for interface %d: CR",path, i);
		...
		usbi_dbg("interface number = %d",interface_number);
		...
		for (j=0; j<nb_paths; j++) {
			usbi_dbg("%d: checking path '%s' against short path '%s'",j,sanitized_path[j],sanitized_short);
...
static int windows_open(struct libusb_device_handle *dev_handle)
{
	...
	r = priv->apib->open(dev_handle);

	/* The orginal libusb0.sys doesn't do a set_config because it might be acting */
	/* as a filter driver, but it is assumed that the one we're working with here */
	/* does. To make sure that it will work with kdrivers based on a libusb0 that */
	/* doesn't do a set_config (e.g. ptlibusb0, libusb0 filter driver ?) */
	/* do one now. */
	if (r == LIBUSB_SUCCESS && priv->active_config == 0 && priv->apib->id == USB_API_LIBUSB0) {
		r = libusb0_set_config(dev_handle, 1);
	}
	return r;
}
...
static int windows_resetep(struct libusb_device_handle *dev_handle, unsigned char endpoint)
{
	struct windows_device_priv *priv = __device_priv(dev_handle->dev);
	return priv->apib->resetep(dev_handle, endpoint);
}
...
static void windows_clear_transfer_priv(struct usbi_transfer *itransfer)
{
	...
	const struct winfd *wfd = transfer_priv->pollable_fd;
	transfer_priv->pollable_fd = &INVALID_WINFD;		// Because we aren't locking
	usbi_free_fd(wfd->fd);								// Then free
...
int windows_cancel_transfer(struct usbi_transfer *itransfer)
{
	...
	if (itransfer->flags & USBI_TRANSFER_TIMED_OUT) {
		int i;
		for (i = 0; i < transfer_priv->pollable_fd->num_urbs; i++)
			transfer_priv->pollable_fd->urbs[i].overlapped->Internal &= ~STATUS_PENDING;
	}
#endif
	switch (transfer->type) {
	case LIBUSB_TRANSFER_TYPE_CONTROL:
...
static int windows_handle_events(struct libusb_context *ctx, struct pollfd *fds, nfds_t nfds, int num_ready)
{
	...
	for (i = 0; i < nfds && num_ready > 0; i++) {
		...
		list_for_each_entry(transfer, &ctx->flying_transfers, list, struct usbi_transfer) {
			...
			// If the usbi_transfer has been added to flying_transfers but hasn't been
			// initialize yet by the back end, transfer_priv might still be 0
			if (transfer_priv->pollable_fd != NULL
			 && transfer_priv->pollable_fd->fd == fds[i].fd) {
				found = true;
				break;
			}
		}
		...
		if (found) {
			io_result = transfer_priv->pollable_fd->io_result;
			io_size = transfer_priv->pollable_fd->io_size;
...
unsigned __stdcall windows_clock_gettime_threaded(void* param)
{
	...
	/* GWG - don't trust the QPF! */
	if (1 || !QueryPerformanceFrequency(&li_frequency)) {
...
static int unsupported_resetep(struct libusb_device_handle *dev_handle, unsigned char endpoint) {
	PRINT_UNSUPPORTED_API(resetep);
}
...
const char* composite_driver_names[] = {"usbccgp", "usbhub"};
...
static int winusb_submit_control_transfer(struct usbi_transfer *itransfer)
{
	...
	wfd = usbi_create_fd(winusb_handle, _O_RDONLY, 1);	
	if (wfd == NULL || wfd->fd < 0) {
		return LIBUSB_ERROR_NO_MEM;
	}
	wfd->itransfer = itransfer;

	wfd->urbs[0].req_size = size;
	if (!WinUsb_ControlTransfer(wfd->handle, *setup, transfer->buffer + LIBUSB_CONTROL_SETUP_SIZE, size, &wfd->urbs[0].sync_size, &wfd->urbs[0].overlapped)) {
...
static int winusb_submit_bulk_transfer(struct usbi_transfer *itransfer)
{
	...
	wfd = usbi_create_fd(winusb_handle, direction_in?_O_RDONLY:_O_WRONLY, 1);
	if (wfd == NULL || wfd->fd < 0) {
		return LIBUSB_ERROR_NO_MEM;
	}
	wfd->itransfer = itransfer;

	wfd->urbs[0].req_size = transfer->length;
	if (direction_in) {
		usbi_dbg("reading %d bytes", transfer->length);
		ret = WinUsb_ReadPipe(winusb_handle, transfer->endpoint, transfer->buffer, transfer->length, &wfd->urbs[0].sync_size, &wfd->urbs[0].overlapped);
	} else {
		usbi_dbg("writing %d bytes", transfer->length);
		ret = WinUsb_WritePipe(winusb_handle, transfer->endpoint, transfer->buffer, transfer->length, &wfd->urbs[0].sync_size, &wfd->urbs[0].overlapped);
	}
	if (!ret) {
		if(GetLastError() != ERROR_IO_PENDING) {
...
static int winusb_reset_device(struct libusb_device_handle *dev_handle)
{
	...
	for (i=0; i<USB_MAXINTERFACES; i++) {
		...
		for (wfd = handle_to_winfd(winusb_handle); wfd != NULL;)
		{
                        ...
                        void *in, int in_size, int *ret)
{
	...
	if (!DeviceIoControl(dev, code, out, out_size, in, in_size, &_ret, &ol)) {
		if(GetLastError() != ERROR_IO_PENDING) {
...
/* Special internal to set config if libusb0.sys hasn't done it */
static int libusb0_set_config(struct libusb_device_handle *dev_handle, int config) {
	struct libusb_context *ctx = DEVICE_CTX(dev_handle->dev);
	struct windows_device_handle_priv *handle_priv = (struct windows_device_handle_priv *)dev_handle->os_priv;
	struct windows_device_priv *priv = __device_priv(dev_handle->dev);
	...
	if ((ret = libusb0_io_sync(libusb0_handle, LIBUSB0_IOCTL_SET_CONFIGURATION, 
...
static int libusb0_submit_iso_transfer(struct usbi_transfer *itransfer) {
	 ...
	 * transfers of same sized packets, or fix the libusb0.sys kernel driver
	...
	/* libusb0 places a limit on bulk URBs. we divide up larger requests
	 * into smaller units to meet such restriction, then fire off all the
	 * units at once. */ 

	max_size = direction_in ? LIBUSB0_MAX_READ_WRITE : LIBUSB0_MAX_READ_WRITE;

	num_urbs = (transfer->length + max_size-1) / max_size;
	if (num_urbs == 0)
		num_urbs++;		// Special case, one zero length transfer

	wfd = usbi_create_fd(libusb0_handle, direction_in?_O_RDONLY:_O_WRONLY, num_urbs);	
	if (wfd == NULL || wfd->fd < 0) {
		return LIBUSB_ERROR_NO_MEM;
	}
	wfd->itransfer = itransfer;
	...
	for (i = 0; i < num_urbs; i++) {
		...
		if (direction_in) {
			...
			ret = DeviceIoControl(libusb0_handle, 
                      ...
                      &req, sizeof(libusb0_request), 
                      buffer,
                      tlen, &wfd->urbs[i].sync_size, &wfd->urbs[i].overlapped);
		} else {
...
static int libusb0_submit_control_transfer(struct usbi_transfer *itransfer)
{
	...
	req_size = sizeof(libusb0_request) + ((request_type & LIBUSB_ENDPOINT_IN) ? 0 : size);
	...
	wfd = usbi_create_fd(libusb0_handle, _O_RDONLY, 1);	
	if (wfd == NULL || wfd->fd < 0) {
		return LIBUSB_ERROR_NO_MEM;
	}
	wfd->itransfer = itransfer;

	usbi_dbg("libusb0_control %s %d bytes fd %d", (request_type & LIBUSB_ENDPOINT_IN) ? "read" : "write", 
	(request_type & LIBUSB_ENDPOINT_IN) ?  in_size : size, wfd->fd);

	wfd->urbs[0].req_size = (request_type & LIBUSB_ENDPOINT_IN) ?  in_size : size;

	if (!DeviceIoControl(wfd->handle, code, req, req_size, data, in_size, &wfd->urbs[0].sync_size, &wfd->urbs[0].overlapped)) {
		if(GetLastError() != ERROR_IO_PENDING) {
		...
		wfd->urbs[0].sync_compl = TRUE;
		if (!(request_type & LIBUSB_ENDPOINT_IN))
			wfd->urbs[0].cntr_extra_size = size;
	}
...
static int libusb0_submit_bulk_transfer(struct usbi_transfer *itransfer)
{
	...
	for (i = 0; i < num_urbs; i++) {
		...
		wfd->urbs[i].req_size = tlen;
		if (direction_in) {
			usbi_dbg("reading bulk %d bytes from handle %d fd %d ep %02X urb %d/%d", tlen, wfd->handle, wfd->fd, transfer->endpoint, i+1, num_urbs);
			ret = DeviceIoControl(libusb0_handle, 
			...
			usbi_dbg("writing bulk %d bytes from handle %d fp %d ep %02X urb %d/%d", tlen, wfd->handle, wfd->fd, transfer->endpoint, i+1, num_urbs);
			ret = DeviceIoControl(libusb0_handle, 
...
static int libusb0_resetep(struct libusb_device_handle *dev_handle, unsigned char endpoint)
{
	...
	if ((ret = libusb0_io_sync(libusb0_handle, LIBUSB0_IOCTL_RESET_ENDPOINT, &req, 
	                    sizeof(libusb0_request), NULL, 0, NULL)) != LIBUSB_SUCCESS) {
		usbi_err(ctx, "libusb0_resetep: 0x%02x failed: %s", endpoint, libusb_strerror(ret));
		return ret;
	}
...
static int hid_submit_control_transfer(struct usbi_transfer *itransfer)
{
	...
	wfd = usbi_create_fd(hid_handle, _O_RDONLY, 1);	
	if (wfd == NULL || wfd->fd < 0) {
		return LIBUSB_ERROR_NO_MEM;
	}
	wfd->itransfer = itransfer;
...
static int hid_submit_bulk_transfer(struct usbi_transfer *itransfer) {
	...
	wfd = usbi_create_fd(hid_handle, direction_in?_O_RDONLY:_O_WRONLY, 1);	
	if (wfd == NULL || wfd->fd < 0) {
		return LIBUSB_ERROR_NO_MEM;
	}
	wfd->itransfer = itransfer;
	...
	if (direction_in) {
		...
		ret = ReadFile(hid_handle, transfer_priv->hid_buffer, transfer->length+1, &size, &wfd->urbs[0].overlapped);
	} else {
		...
		ret = WriteFile(hid_handle, transfer_priv->hid_buffer, transfer->length+1, &size, &wfd->urbs[0].overlapped);
	}
	if (!ret) {
		if (GetLastError() != ERROR_IO_PENDING) {
...
static int composite_resetep(struct libusb_device_handle *dev_handle, unsigned char endpoint)
{
	...
	return priv->usb_interface[current_interface].apib->resetep(dev_handle, endpoint);
}
