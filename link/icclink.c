...
#ifdef XYZCURVE
			p->out.devlin->invlin(p->out.devlin, tdevv, tdevv);
#endif
		}
		if (p->out.locus) {
...
main(int argc, char *argv[]) {
	...
	/* Setup the profile color lookup information */
	{
		...
		if (li.mode > 0 && (li.gmi.usecas & 0xff) != 0) {
		...
		if (oalg != icmLutType || li.mode < 2) {	/* Using B2A table or inv. mono/matrix */
			...
			if (li.out.inking == 7) {
				if ((li.out.b2aluo = li.out.x->get_luobj(li.out.x, 0, icmBwd, li.out.intent,
		                              li.pcsor, icmLuOrdNorm, &li.out.vc, NULL)) == NULL) {
...
#define DBGNO 2		/* Up to 10 */
			...
			if (li.verb) {
				unsigned int ui;
				for (li.total = 1, ui = 0; ui < wo->inputChan; ui++, li.total *= wo->clutPoints)
					; 
