...
/*
 * Clut per channel curve fitting - algorithm 2
 *
 ...
 * to create the input, sub-grid and output curves.
 *
 */
...
static double xfit2_subcurve(xfit2 *p, double in, int chan) {
	double rv;
	int i;
...
static double xfit2_invsubcurve(xfit2 *p, double in, int chan) {
	double rv;
	int i;
...
static double xfit2_incurve(xfit2 *p, double in, int chan) {
	double rv;
	if (p->tcomb & oc2_i)
	...
	if (p->tcomb & oc2_s)
		rv = xfit2_subcurve(p, rv, chan);
	return rv;
}

/* Inverse Lookup a value though the input and sub-grid curve */
static double xfit2_invincurve(xfit2 *p, double in, int chan) {
	double rv;
	if (p->tcomb & oc2_s)
		rv = xfit2_invsubcurve(p, in, chan);
	else
		rv = in;
	if (p->tcomb & oc2_i)
...
static double xfit2_outcurve(xfit2 *p, double in, int chan) {
	double rv;
	if (p->tcomb & oc2_o)
...
static double xfit2_invoutcurve(xfit2 *p, double in, int chan) {
	double rv;
	if (p->tcomb & oc2_o)
...
/* output curves in xfit2, estimate the resulting linear */
/* interpolation error. */
static double span_err(
	...
	if (ntp > MAX_NTP)
		error("xfit2 span_err got ntp > %d",MAX_NTP);
	...
	for (ee = 0; ee < di;) {		/* Until index completed */
		...
		for (n = 0; n < ntp; n++) {	/* Lookup points along span */
			...
			for (e = 0; e < p->di; e++) {
				...
				if (p->tcomb & oc2_s)
					lupd[n].p[e] = xfit2_subcurve(p, lupd[n].p[e], e);
			}
			Sx += lupd[n].p[ch];
		...
		if (p->flags & XFIT_FM_INPUT) {	/* x as a function of y: x = a + by  */
			...
			for (n = 0; n < ntp; n++) {
				...
				for (f = 0; f < p->fdi; f++) {
					...
					if (p->tcomb & oc2_s)
						lp[ch] = xfit2_invsubcurve(p, lp[ch], ch);
...
/* Given a grid cell span, and using the current input and */
/* output curves in xfit2, allocate and initialize all the test points */
...
//printf("~1 span_setup ch %d, span index %d, %f-%f\n",ch,sx,x0,x1);
	...
	if (p->ctp == NULL) {
		if ((p->ctp = (xfit2_ctp **)calloc(p->di, sizeof(xfit2_ctp *))) == NULL)
			return 2;
	}
	if (p->ctp[ch] == NULL) {
		if ((p->ctp[ch] = (xfit2_ctp *)calloc(p->gres[ch]-1, sizeof(xfit2_ctp))) == NULL)
			return 2;
	}
	ctp = &p->ctp[ch][sx];
	...
	if (ctp->lup == NULL) {
		...
		if (ctp->ns != ns || ctp->ntp != ntp
		 || ctp->x0 != x0 || ctp->x1 != x1) {
			warning("xfit2, span_setup assert: ns or np has changed");
			return 1;
		}
	}
...
static double out_span_err(
	xfit2 *p,
	int ch,					/* Axis being evaluated */
	xfit2_ctp *ctp			/* test points for this span on this axis */
) {
	int di = p->di;
	int fdi = p->fdi;
	int i, j, ee, e, f, n;
...
static double xfit2ifunc(void *edata, double *v) {
	xfit2 *p = (xfit2 *)edata;
	int i;
	double *pp, de = 0.0, rv;
	double pgx, gx;	
...
//printf("~1 span %f -> %f : uer = %f, rv = %f\n",pgx,gx,p->uerrv[i],ser);
	...
	rv = de + avudev + smv;
#ifdef DEBUG
printf("~1 xfit2ifunc returning (sm %f, ue %f, de %f) %f\n",smv, avudev, de, rv);
#endif
...
static double dxfit2ifunc(void *edata, double *dv, double *v) {
	xfit2 *p = (xfit2 *)edata;
	int i;
	double rv;

	rv = xfit2ifunc(edata, v);
	...
	for (i = 0; i < p->opt_cnt; i++) {
		...
		v[i] += 1e-7;
		dv[i] = (xfit2ifunc(edata, v) - rv)/1e-7;
		v[i] -= 1e-7;
	}
	return rv;
}
...
static double xfit2ofunc(void *edata, double *v) {
	xfit2 *p = (xfit2 *)edata;
	int e, i;
	double *pp, de = 0.0, rv;
	double pgx, gx;	
...
//printf("~1 span %f -> %f : rv = %f\n",pgx,gx,ser);
	...
	rv = de + smv;
#ifdef DEBUG
printf("~1 xfit2ofunc returning (sm %f, de %f) %f\n",smv, de, rv);
#endif
...
/* Fake version of above that simulate partial de's for conjgrad */
static double dxfit2ofunc(void *edata, double *dv, double *v) {
	xfit2 *p = (xfit2 *)edata;
	int i;
	double rv;

	rv = xfit2ofunc(edata, v);
	...
	for (i = 0; i < p->opt_cnt; i++) {
		...
		v[i] += 1e-7;
		dv[i] = (xfit2ofunc(edata, v) - rv)/1e-7;
		v[i] -= 1e-7;
	}
	return rv;
}
...
static double xfit2ofunc2(void *edata, double *v) {
	xfit2 *p = (xfit2 *)edata;
	int e, i, j;
	double *pp, de = 0.0, rv;
	double pgx, gx;	
...
/* Fake version of above that simulate partial de's for conjgrad */
static double dxfit2ofunc2(void *edata, double *dv, double *v) {
	xfit2 *p = (xfit2 *)edata;
	int i;
	double rv;

	rv = xfit2ofunc2(edata, v);
	...
	for (i = 0; i < p->opt_cnt; i++) {
		...
		v[i] += 1e-7;
		dv[i] = (xfit2ofunc2(edata, v) - rv)/1e-7;
		v[i] -= 1e-7;
	}
	return rv;
}
...
static double xfit2sfunc2(void *edata, double *v) {
	xfit2 *p = (xfit2 *)edata;
	int e, i, j;
	double *pp, de = 0.0, rv;
	double pgx, gx;	
...
//	rv = de + smv;
	rv = de;
#ifdef DEBUG
printf("~1 xfit2sfunc2 returning (sm %f, de %f) %e\n",smv, de, rv);
#endif
...
/* Fake version of above that simulate partial de's for conjgrad */
static double dxfit2sfunc2(void *edata, double *dv, double *v) {
	xfit2 *p = (xfit2 *)edata;
	int i;
	double rv;

	rv = xfit2sfunc2(edata, v);
	...
	for (i = 0; i < p->opt_cnt; i++) {
		...
		v[i] += 1e-7;
		dv[i] = (xfit2sfunc2(edata, v) - rv)/1e-7;
		v[i] -= 1e-7;
	}
	return rv;
}
...
static double symoptfunc(void *edata, double *v) {
	...
	/* Copy the parameter being tested back into xfit2 */
...
printf("~1 done\n");
	...
	if ((p->tcomb & oc2_i) != 0 ) {
		...
		for (e = 0; e < di; e++) {
			...
			p->optt = 0;
			p->och = e;
			setup_xfit2(p, p->wv, p->sa, 2.0); 
#ifdef NEVER
			if (powell(NULL, p->opt_cnt, p->wv, p->sa, POWTOL, MAXITS, xfit2ifunc, (void *)p) != 0)
				warning("set_icxLuLut: Powell failed to converge");
#else
			/* fake conjgrad to see how speed is */
			if (conjgrad(NULL, p->opt_cnt, p->wv, p->sa, POWTOL, MAXITS, xfit2ifunc, dxfit2ifunc, (void *)p) != 0)
				warning("set_icxLuLut: Powell failed to converge");
#endif
...
printf("~1 optimising sub grid axis %d, grid %d\n",e,n);
				setup_xfit2(p, p->wv, p->sa, 2.0); 
#ifdef NEVER
				if (powell(NULL, p->opt_cnt, p->wv, p->sa, POWTOL, MAXITS, xfit2sfunc2, (void *)p) != 0)
					warning("set_icxLuLut: Powell failed to converge");
#else
				if (conjgrad(NULL, p->opt_cnt, p->wv, p->sa, POWTOL, MAXITS, xfit2sfunc2, dxfit2sfunc2, (void *)p) != 0)
					warning("set_icxLuLut: Powell failed to converge");
#endif
				for (i = 0; i < p->opt_cnt; i++)		/* Copy optimised values back */
					p->v[p->opt_off + i] = p->wv[i];
...
#ifdef NEVER
			if (powell(NULL, p->opt_cnt, p->wv, p->sa, POWTOL, MAXITS, xfit2ofunc2, (void *)p) != 0)
				warning("set_icxLuLut: Powell failed to converge");
#else
			if (conjgrad(NULL, p->opt_cnt, p->wv, p->sa, POWTOL, MAXITS, xfit2ofunc2, dxfit2ofunc2, (void *)p) != 0)
				warning("set_icxLuLut: Powell failed to converge");
#endif
			for (i = 0; i < p->opt_cnt; i++)		/* Copy optimised values back */
				p->v[p->opt_off + i] = p->wv[i];
...
/* We're done with an xfit2 */
static void xfit2_del(xfit2 *p) {
	int e;
	if (p->v != NULL)
		free(p->v);
	if (p->wv != NULL)
...
xfit2 *new_xfit2(
	...
	if ((p = (xfit2 *)calloc(1, sizeof(xfit2))) == NULL) {
		return NULL;
	}

	/* Set method pointers */
	p->fit         = xfit2_fit;
	p->incurve     = xfit2_incurve;
	p->invincurve  = xfit2_invincurve;
	p->outcurve    = xfit2_outcurve;
	p->invoutcurve = xfit2_invoutcurve;
	p->del         = xfit2_del;
...
/* ========================================================= */
/* 1D per sample point RSPL support */
