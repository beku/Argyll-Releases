...
typedef enum {
	oc3_i   = 1,			/* Input */
	oc3_s   = 2,			/* Input sub-grid */
	oc3_o   = 4,			/* Output */
	oc3_iso = 1 + 2 + 4,	/* Input, Sub-grid and Output */
} optcomb3;
...
struct _xfit3 {
	...
	xfit3_ctp **ctp; 		/* Pre-computed test/ref points ctp[di][gres-1] */

	/* Methods */
	void (*del)(struct _xfit3 *p);
	...
	double (*incurve)(struct _xfit3 *p, double in, int chan);

	/* Inverse Lookup a value though an input curve */
	double (*invincurve)(struct _xfit3 *p, double in, int chan);

	/* Lookup a value though an output curve */
	double (*outcurve)(struct _xfit3 *p, double in, int chan);

	/* Inverse Lookup a value though an output curve */
	double (*invoutcurve)(struct _xfit3 *p, double in, int chan);

}; typedef struct _xfit3 xfit3;
